"""Tests for the concrete PPTX-to-PDF workflow."""
...
import docuforge.converters.office.pptx_to_pdf as pptx_module
...
from docuforge.converters.office import (
    PptxToPdfConverter as OfficePptxToPdfConverter,
)
from docuforge.converters.office import (
    PptxToPdfRequest as OfficePptxToPdfRequest,
)
from docuforge.converters.office import (
    convert_pptx_to_pdf as office_convert_pptx_to_pdf,
)
...
def make_request(
    tmp_path: Path,
    *,
    input_name: str = "source.pptx",
    output_name: str = "result.pdf",
) -> PptxToPdfRequest:
    input_path = tmp_path / input_name
    write_minimal_pptx(input_path)
    return PptxToPdfRequest(input_path, tmp_path / output_name)


def write_minimal_pptx(
    path: Path,
    *,
    members: tuple[str, ...] = ("[Content_Types].xml", "ppt/presentation.xml"),
) -> None:
...
def test_request_has_intrinsic_single_pptx_to_pdf_identity() -> None:
    input_path = Path("source.pptx")
    output_path = Path("custom.pdf")

    request = PptxToPdfRequest(input_path, output_path)

    assert request.input_paths == (input_path,)
    assert request.input_path == input_path
    assert request.output_path == output_path
    assert request.source_format is DocumentFormat.PPTX
...
def test_converter_identity_and_injected_engine() -> None:
    ...
    converter = PptxToPdfConverter(engine)

    assert converter.operation is ConversionOperation.CONVERT
    assert converter.source_format is DocumentFormat.PPTX
...
def test_converter_rejects_object_without_engine_contract() -> None:
    with pytest.raises(TypeError, match="OfficeConversionEngine"):
        PptxToPdfConverter(object())  # type: ignore[arg-type]


def test_office_and_top_level_exports_share_the_same_api() -> None:
    assert OfficePptxToPdfRequest is PptxToPdfRequest
    assert OfficePptxToPdfConverter is PptxToPdfConverter
    assert office_convert_pptx_to_pdf is convert_pptx_to_pdf


def test_success_uses_isolated_workspace_and_exact_custom_destination(tmp_path: Path) -> None:
    request = make_request(
        tmp_path,
        input_name="r̩sum̩ draft.pptx",
        output_name="final client r̩sum̩.pdf",
    )
    engine = FakeOfficeEngine()

    result = PptxToPdfConverter(engine).convert(request)
    ...
    assert engine.workspaces[0].name.startswith(".docuforge-pptx-")
...
def test_public_helper_delegates_through_injected_engine(tmp_path: Path) -> None:
    ...
    assert convert_pptx_to_pdf(request, engine=engine) == request.output_path
    assert len(engine.calls) == 1


@pytest.mark.parametrize("suffix", [".ppt", ".docx", ".xlsx", ".txt"])
def test_non_pptx_source_suffixes_are_rejected(tmp_path: Path, suffix: str) -> None:
    request = make_request(tmp_path, input_name=f"source{suffix}")
    engine = FakeOfficeEngine()

    with pytest.raises(InvalidConversionRequestError, match=".pptx"):
        PptxToPdfConverter(engine).convert(request)

    assert engine.calls == []


def test_uppercase_pptx_and_pdf_suffixes_are_accepted(tmp_path: Path) -> None:
    request = make_request(tmp_path, input_name="SOURCE.PPTX", output_name="RESULT.PDF")

    assert PptxToPdfConverter(FakeOfficeEngine()).convert(request) == request.output_path
...
def test_renamed_non_pptx_content_is_rejected_before_engine_call(
    tmp_path: Path,
    payload: bytes,
) -> None:
    input_path = tmp_path / "renamed.pptx"
    input_path.write_bytes(payload)
    engine = FakeOfficeEngine()

    with pytest.raises(InvalidConversionRequestError, match="valid PPTX"):
        PptxToPdfConverter(engine).convert(
            PptxToPdfRequest(input_path, tmp_path / "result.pdf")
        )
...
def test_incomplete_pptx_package_is_rejected_before_engine_call(
    tmp_path: Path,
    members: tuple[str, ...],
) -> None:
    input_path = tmp_path / "incomplete.pptx"
    write_minimal_pptx(input_path, members=members)
    engine = FakeOfficeEngine()
...
def test_missing_source_is_rejected_before_engine_call(tmp_path: Path) -> None:
    request = PptxToPdfRequest(tmp_path / "missing.pptx", tmp_path / "result.pdf")
    engine = FakeOfficeEngine()
...
def test_directory_source_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConversionRequestError, match="not a file"):
        PptxToPdfConverter(FakeOfficeEngine()).convert(
            PptxToPdfRequest(source, tmp_path / "result.pdf")
        )


def test_missing_output_parent_is_rejected(tmp_path: Path) -> None:
    request = make_request(tmp_path)
    request = PptxToPdfRequest(request.input_path, tmp_path / "missing" / "result.pdf")

    with pytest.raises(InvalidConversionRequestError, match="existing directory"):
        PptxToPdfConverter(FakeOfficeEngine()).convert(request)
...
def test_output_parent_file_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConversionRequestError, match="existing directory"):
        PptxToPdfConverter(FakeOfficeEngine()).convert(
            PptxToPdfRequest(request.input_path, parent_file / "result.pdf")
        )
...
def test_directory_output_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConversionRequestError, match="is a directory"):
        PptxToPdfConverter(FakeOfficeEngine()).convert(PptxToPdfRequest(request.input_path, output))


@pytest.mark.parametrize("output_name", ["result", "result.pptx", "result.png"])
...
def test_same_filesystem_object_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidConversionRequestError, match="different files"):
        PptxToPdfConverter(FakeOfficeEngine()).convert(
            PptxToPdfRequest(request.input_path, linked_output)
        )


def test_wrong_request_type_and_generic_request_are_rejected(tmp_path: Path) -> None:
    converter = PptxToPdfConverter(FakeOfficeEngine())
    ...
    with pytest.raises(InvalidConversionRequestError, match="PptxToPdfRequest"):
...
def test_wrong_request_identity_is_rejected(
    ...
    with pytest.raises(UnsupportedConversionError):
        PptxToPdfConverter(FakeOfficeEngine()).convert(wrong_identity)
...
def test_wrong_engine_format_identity_is_rejected(
    ...
    with pytest.raises(OfficeConversionError, match=message):
        PptxToPdfConverter(FakeOfficeEngine(wrong_format)).convert(request)
...
def test_non_result_object_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfficeConversionError, match="invalid conversion result"):
        PptxToPdfConverter(FakeOfficeEngine(lambda _: object())).convert(request)
...
def test_symlink_to_output_outside_workspace_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfficeConversionError, match="invalid PDF artifact"):
        PptxToPdfConverter(FakeOfficeEngine(symlink_result)).convert(request)
...
def test_missing_engine_artifact_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfficeConversionError, match="usable PDF"):
        PptxToPdfConverter(FakeOfficeEngine(missing)).convert(request)
...
def test_engine_directory_artifact_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfficeConversionError, match="invalid PDF artifact"):
        PptxToPdfConverter(FakeOfficeEngine(directory)).convert(request)
...
def test_engine_special_artifact_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(OfficeConversionError, match="invalid PDF artifact"):
        PptxToPdfConverter(FakeOfficeEngine(special)).convert(request)
...
def test_publication_failure_is_safe_and_preserves_destination(
    ...
    monkeypatch.setattr(pptx_module.os, "replace", fail_publication)
