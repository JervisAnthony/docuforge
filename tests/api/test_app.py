...
def test_create_app_returns_separate_fastapi_instances() -> None:
    ...
    assert first.version == "0.1.0.dev0"
...
def test_default_version_is_consistent_across_application_and_routes() -> None:
    ...
    assert settings.application_version == package_version()
    assert application.version == settings.application_version
    assert client.get("/api/v1").json()["version"] == settings.application_version
    assert client.get("/api/v1/health").json()["version"] == settings.application_version
