"""Low-level converter for extracting selected pages from one PDF."""
...
from docuforge.converters.pdf.models import PdfExtractPagesRequest
...
class PdfExtractPagesConverter(Converter):
    """Extract explicitly selected pages in request order."""

    def __init__(self) -> None:
        """Initialize the PDF-to-PDF page-extraction converter."""
        super().__init__(
    ...
    def convert(self, request: ConversionRequest) -> tuple[Path, ...]:
        """Extract requested pages and atomically replace the single output."""
        self._validate_request(request)
        assert isinstance(request, PdfExtractPagesRequest)
        ...
        try:
            ...
            raise PdfProcessingError(
                "Unable to extract the requested PDF pages."
            ) from error
        finally:
            writer.close()
    ...
    def _validate_request(request: ConversionRequest) -> None:
        ...
        if (
            ...
            raise UnsupportedConversionError(
                "PdfExtractPagesConverter requires a PDF page-extraction request."
            )
        if not isinstance(request, PdfExtractPagesRequest):
            raise InvalidConversionRequestError(
                "PDF page extraction requires a PdfExtractPagesRequest."
            )
